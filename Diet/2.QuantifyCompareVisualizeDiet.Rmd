...
comp <- batsbugszBM %>% dplyr::group_by(bat.sp, Species) %>% dplyr::summarize(sum = sum(TotalAbundance)) %>% dplyr::mutate(Species = factor(Species), bat.sp = factor(bat.sp)) # 192 obs (156 distinct levels, so there are at least 36 prey that overlap) 

comp1 <- comp %>% dplyr::group_by(Species) %>% dplyr::summarize(bats = list(bat.sp))
...
preytaxa <- batsbugszBM %>% dplyr::select(Species, Order) %>% dplyr::distinct() 

comp3 <- dplyr::left_join(comp2, preytaxa)

comp4 <- comp3 %>% dplyr::select(order(colnames(comp3)))
com4 <- comp4 %>% dplyr::arrange(Order)
...
prey3 <- prey1 %>% dplyr::group_by(BatSpecies, Sample, Family, Order) %>% 
    dplyr::summarize(FamAbun = sum(TotalAbundance)) ## 1798 obs

checkthis <- prey3 %>% 
  dplyr::select(Family, Order) %>% 
  dplyr::distinct() %>% dplyr::arrange(Order) 
...
p <-  ggplot2::ggplot() + 
  ggplot2::geom_count(data = prey3, 
                      ggplot2::aes(x = BatSpecies, y = Family, 
                                   size = FamAbun, color = Order), 
                      alpha = 0.9,) + 
  ggplot2::scale_size_continuous(range = c(5, 12)) +
  ggplot2::scale_color_manual(values = MetBrewer::met.brewer("Signac", n = 10)) +
  ...
  ggplot2::geom_jitter() + ggplot2::theme(legend.title = ggplot2::element_blank()) +
...
p <-  ggplot2::ggplot() + 
  ggplot2::geom_point(data = prey3, 
                      ggplot2::aes(x = BatSpecies, y = Family, color = Order), 
                      size = 6) + 
...
microbiome::summarize_phyloseq(bby)
...
quantile(colSums(phyloseq::otu_table(bby)))
#   0%  25%  50%  75% 100% 
# 3857 3934 3950 3961 4000 



bbydf <- phyloseq::psmelt(bby) # 81180 obs of 19 vars
bbydf$Order <- as.factor(bbydf$Order) 
bbydf <- bbydf %>% droplevels() 

bbydf <- bbydf %>% dplyr::mutate(across( c(Sample, bat.sp, Order, Family, Genus, Species), factor)) 
str(bbydf)
summary(bbydf)
...
b17 <- bbydf %>% dplyr::filter(year == 2017) %>% droplevels() # 15 samples
b17$fdate <- as.factor(b17$date) # 4 distinct nights (many missing)

b18 <- bbydf %>% dplyr::filter(year == 2018) %>% droplevels() # 30 samples 
b18$fdate <- as.factor(b18$date) # 7 distinct nights 

## Number of unique samples per year per bat species 
B17 <- bbydf %>% dplyr::filter(year == 2017 & bat.sp == "MBRA") %>% droplevels() 
# 3 samples
levels(B17$Sample)
# "1B" "3B" "4B"


B18 <- bbydf %>% dplyr::filter(year == 2018 & bat.sp == "MBRA") %>% droplevels() 
# 11 samples 
levels(B18$Sample)
# "33B" "35B" "36A" "37B" "38B" "39B" "40A" "41B" "42B" "43B" "44B"

M17 <- bbydf %>% dplyr::filter(year == 2017 & bat.sp == "MMYS") %>% droplevels() 
# 12 samples
levels(M17$Sample)
# "10B" "11B" "12B" "13B" "14B" "15B" "16B" "17B" "5B"  "7B"  "8B"  "9B" 

M18 <- bbydf %>% dplyr::filter(year == 2018 & bat.sp == "MMYS") %>% droplevels() 
# 19 samples 
levels(M18$Sample)
...
## MBRA
batsbugszB <- batsbugszB %>% dplyr::mutate(across( c(Sample, bat.sp, BatSpecies, Order, Family, Genus, Species), factor)) 
str(batsbugszB)
...
## MMYS 
batsbugszM <- batsbugszM %>% dplyr::mutate(across( c(Sample, bat.sp, BatSpecies, Order, Family, Genus, Species), factor)) 
str(batsbugszM)
...
host_species <- unique(as.character(phyloseq::sample_data(bby)$bat.sp))
host_species
# "MMYS" "MBRA"

meta <- data.frame(phyloseq::sample_data(bby))
host_species <- unique(as.character(meta$bat.sp))
host_species

# split int oa list of phyloseq objects
meta <- data.frame(phyloseq::sample_data(bby))

ps_list <- lapply(host_species, function(species_name) {
  sample_ids <- rownames(meta)[meta$bat.sp == species_name]
  phyloseq::prune_samples(sample_ids, bby)
})

names(ps_list) <- host_species

## Now separate taxa 
ps1 <- phyloseq::subset_samples(bby, bat.sp == "MBRA")
ps2 <- phyloseq::subset_samples(bby, bat.sp == "MMYS")

## and prune out zero observations for diet taxa 
ps_list <- lapply(ps_list, function(p) {
  phyloseq::prune_taxa(phyloseq::taxa_sums(p) > 0, p)
})

# Convert phylsoeq to Presence Absence data
ps_pa_list <- lapply(ps_list, function(p) {
  phyloseq::transform_sample_counts(p, function(x) ifelse(x > 0, 1, 0))
})

# Convert each phyloseq obecjt to an OTU matrix
otu_list <- lapply(ps_pa_list, function(p) {
  mat <- as(phyloseq::otu_table(p), "matrix")
  
  if (phyloseq::taxa_are_rows(p)) {
    mat <- t(mat)
  }
...
## specaccum() function in vegan - species accumulation 
## As I collect more samples from this host species, how many additional taxa do I observe?
# y-axis = cumulative number of observed ASVs/OTUs/genera/families
# method "random" - randomizes sample order many times and averages the accumulation curve across permutations.

accum_list <- lapply(names(otu_list), function(species_name) {
  mat <- otu_list[[species_name]]
  
  acc <- vegan::specaccum(mat, method = "random")
  
  data.frame(
    HostSpecies = species_name,
    Sites = acc$sites,
...
accum_df <- dplyr::bind_rows(accum_list) %>% 
  dplyr::mutate(species = factor(dplyr::case_when(
    HostSpecies == "MBRA" ~"M. brandtii",
    HostSpecies == "MMYS" ~ "M. mystacinus")), 
                SID = factor(Sites)) 
summary(accum_df)
#Sites are unique sample IDs that correspond to individual bats for each species. 

## Now to visualize it: 
rich <- ggplot2::ggplot(
  accum_df,
  ggplot2::aes(x = Sites, y = Richness, color = species, fill = species)
) +
  ggplot2::geom_ribbon(
  ...
  ggplot2::ylab("Taxa richness") + 
  ggplot2::geom_line(linewidth = 1.2) +
  ggplot2::geom_point(size = 2) +
  ggplot2::theme_classic() +
  ggplot2::ggtitle("Taxa richness accumulation curve for bat diet analysis") +
  ggplot2::theme(legend.text = ggplot2::element_text(face = "italic")) +
  ggplot2::scale_fill_manual(values = c("#32a5ff", "#011f4b")) + 
  ggplot2::scale_color_manual(values = c("#32a5ff", "#011f4b")) 

# ggplot2::ggsave(file = file.path(output_today, "taxa accumulation curve.png"), rich, width = 10, height = 7, dpi = 600)

# write.csv(accum_df, file = file.path(output_today, "TaxaAccumulation_df.csv")) 
