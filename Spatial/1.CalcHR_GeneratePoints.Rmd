...
tm_shape(range95Thea_sf) + 
  tm_borders(fill_alpha=.7, col = "#32a5ff", lwd = 2) + 
  tm_fill(fill_alpha=.1, fill= "#32a5ff") +
  tm_scalebar(position = c("top", "right"), width = 200, text.size = 15) 
...
# Different approach to figure 2 
...
## Now more detailed exploration of the individual home ranges 
## Use sattelite images
```{r}
BrandtsHR<-
  tm_shape(Astrid_sf, bbox = bound_box) + 
  tmap_mode('view') +
...
MystacinusHR <- 
  ...
  tmap_mode('view') +
  tm_basemap("Esri.WorldImagery") +
  tm_dots(fill= "white", size = .3, fill_alpha = .8) +
tm_shape(range95Amelia_sf) + 
  tm_borders(fill_alpha=.7, col = "white", lwd = 2) + 
  tm_fill(fill_alpha=.1, fill= "white") +
